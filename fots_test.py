...
if __name__=="__main__":
    ...
    mesh = o3d.io.read_triangle_mesh("assets/daniel/cone.stl")
    model_center = mesh.get_center()

    R = mesh.get_rotation_matrix_from_xyz(rot) 
    mesh.rotate(R, center=model_center) 
    ...
    cx = model_center[0]#np.mean(vertices[:,0])
    cy = model_center[1]#np.mean(vertices[:,1])
    ...
    marker = MarkerMotion(frame0_blur=tact_img,depth=zq/mm_to_pixel,mask=contact_mask,traj=relative_pos,lamb=[0.00125,0.00021,0.00038])
